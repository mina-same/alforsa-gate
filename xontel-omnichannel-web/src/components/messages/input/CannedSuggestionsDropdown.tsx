...
export default function CannedSuggestionsDropdown({
  isOpen,
  isLoading,
  slashQuery,
  responses,
        ...
        {responses.length > 0 ? (
          responses.map((res, idx) => (
            ...
            >
              ...
              <span className="text-xs text-xon-text-primary/80 line-clamp-1">
                {res.content}
              </span>
            </button>
          ))
          ...
          <div className="px-4 py-8 text-center flex flex-col items-center gap-2">
            <div className="p-3 bg-xon-surface-container-hover rounded-full">
              <Command className="h-5 w-5 text-xon-text-secondary/30" />
            </div>
            <p className="text-xs text-xon-text-secondary">
              {slashQuery === ""
                ? "Type to search canned responses..."
                : `No canned responses found for "/${slashQuery}"`}
            </p>
          </div>
        )}
      </div>
