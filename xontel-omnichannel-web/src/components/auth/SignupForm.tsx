...
import { useRegister, type RegisterResponse } from '@api'
...
import { validateSignupForm, parseApiError } from '@/utils/validation'
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!validation.isValid) {
        ...
        setError(firstError || t('error_missing_fields') || 'Please check your inputs')
        return
      }
        ...
        },
        {
          onSuccess: (data: RegisterResponse) => {
            try {
              ...
              setError('Registration successful but navigation failed. Please refresh the page.')
            }
          },
          onError: (err: Error) => {
            try {
              const errorMessage = parseApiError(err)
              setError(errorMessage || t('error_registration_failed') || 'Registration failed')
            } catch (parseError) {
              console.error('Error parsing API response:', parseError)
              setError(t('error_registration_failed') || 'Registration failed')
            }
          },
        }
      )
  ...
  return (
    <div className="min-h-screen bg-xon-surface text-xon-text-primary flex flex-col">
      <div className="grid lg:grid-cols-2 flex-1">
        {/* Left side – Marketing panel with image (same as login) */}
        ...
        <div className="flex flex-col px-12 py-10 bg-xon-surface">
          <div className="flex-1 flex items-center">
            <div className="mx-auto w-full max-w-md space-y-6">
              ...
              <div className="bg-xon-surface-container border border-xon-surface-outline rounded-xl p-6 shadow-md backdrop-blur">
                {isPending ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    {error && (
                      <div className="flex items-start gap-3 p-3 bg-xon-container-red border border-xon-red rounded-lg">
                        <AlertCircle className="h-5 w-5 text-xon-text-red flex-shrink-0 mt-0.5" />
                    ...
                    <div className="space-y-1">
                      <label className={`text-sm font-medium text-xon-text-primary ${isRTL ? 'text-right block' : ''}`} htmlFor="name">
                        {t('full_name_label')}
                      </label>
                      <input
                        id="name"
                    ...
                    <Button
                      type="submit"
                      className="w-full bg-xon-primary text-xon-primary-on hover:opacity-90"
                      disabled={isPending}
                    >
              ...
              {/* Terms & Privacy */}
              <p className="mt-4 text-xs text-xon-text-secondary text-center leading-relaxed">
                {t('terms_agreement_prefix')}{' '}
                <button type="button" className="underline underline-offset-4 hover:text-xon-text-primary">
              ...
              {/* Login link */}
              <p className="text-xs text-xon-text-secondary text-center">
