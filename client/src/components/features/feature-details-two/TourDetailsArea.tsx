  ...
  return (
    <div className="tg-tour-details-video-ratings mb-10">
      {Array.from({ length: full }).map((_, i) => (
        <span key={i}><i className="fa-sharp fa-solid fa-star"></i></span>
      ))}
      {half && <span><i className="fa-sharp fa-solid fa-star-half-stroke"></i></span>}
      {Array.from({ length: 5 - full - (half ? 1 : 0) }).map((_, i) => (
        <span key={`e${i}`}><i className="fa-sharp fa-regular fa-star"></i></span>
      ))}
    </div>
  );
};
...
const TourDetailsArea = () => {
  ...
  const priceUSD = tour.priceStartingFrom?.USD;
  const priceEGP = tour.priceStartingFrom?.EGP;

  const isWishlisted = wishlist.some(item => (item._id ?? item.id) === tour._id);
        ...
        <div className="row align-items-end mb-25">
          <div className="col-xl-9 col-lg-8">
            <div className="tg-tour-details-video-title-wrap">
              {tour.tourType && (
                <span className="tg-listing-item-price-discount shape mb-10 d-inline-block mr-10">
                  {getLang(tour.tourType, lang)}
                </span>
              )}
              {tour.tourStyle && (
                <span className="tg-listing-item-price-discount shape mb-10 d-inline-block" style={{ background: 'var(--tg-color-2, #1a9ae0)' }}>
                  {getLang(tour.tourStyle, lang)}
                </span>
              )}
              <h2 className="tg-tour-details-video-title mb-15">{getLang(tour.heading, lang)}</h2>
              {tour.headingDescription && (
                <p className="mb-10" style={{ color: '#666', fontSize: '16px' }}>
                  {getLang(tour.headingDescription, lang)}
                </p>
              )}
              <div className="tg-tour-details-video-location d-flex flex-wrap align-items-center">
                {priceUSD && (
                  <div className="tg-tour-details-video-feature-price mb-10 mr-25">
                    <p className="mb-0">
                      {t('tour.header.from')} <span>${priceUSD}</span> {t('tour.header.per_person')}
                      {priceEGP && <small className="ml-10" style={{ color: '#888', fontSize: '13px' }}>EGP {priceEGP.toLocaleString()}</small>}
                    </p>
                  </div>
                )}
                {tour.tourLocation && (
                  <span className="mr-25 mb-10">
                    <i className="fa-regular fa-location-dot"></i> {getLang(tour.tourLocation, lang)}
                  </span>
                )}
                {tour.duration && (
                  <span className="mr-25 mb-10">
                    <i className="fa-regular fa-clock"></i> {getLang(tour.duration, lang)}
                  </span>
                )}
              </div>
              <div className="d-flex flex-wrap align-items-center">
                <StarRating rating={tour.averageRating ?? 0} />
                <span className="review ml-10 mb-10">({tour.reviewsCount ?? 0} {t('tour.header.reviews')})</span>
              </div>
            </div>
          </div>
