...
const AboutSlider = () => {
  ...
  const mainImages = tour?.images ?? [];
  const thumbImages = (tour?.gallery && tour.gallery.length > 0) ? tour.gallery : mainImages;

  if (mainImages.length === 0) {
    return <div className="tg-tour-details-gallery-slider-wrap mb-40" style={{ minHeight: 300, background: '#f5f5f5' }} />;
  }
          ...
          <SwiperSlide key={i} className="swiper-slide">
            <div className="tg-tour-details-gallery-thumb">
              <img
                className="w-100"
                src={img.url}
                alt={getLang(img.alt, lang) || getLang(img.title, lang)}
        ...
        <div className="row justify-content-center">
          <div className="col-lg-12">
            <Swiper
              ...
              slidesPerView={Math.min(5, thumbImages.length)}
              spaceBetween={20}
              freeMode={true}
